                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            ...
                            <form @submit.prevent="SubmitReply()">
                                <div class="modal-body">
                                    <input v-model="form2.reply" type="text" name="reply" class="form-control" placeholder="reply..">
                                    <input type="hidden" v-model="form2.comment_id" name="comment_id" id="commentID">
                                </div>
...
<script>
    export default {
        ...
        methods:{
            LoadActiveUser(){
                axios.post('/stud/User/ActiveUser')
                .then(({ data }) => { this.ActiveUser = data });
            },
            LoadArticleDetails(){
                axios.get('/stud/community/article-details/'+this.articleID)
                .then(({ data }) => { this.ArticleDetails = data });
            },
            LoadComments(){
                axios.get('/stud/community/article-comments/'+this.articleID)
                .then(({ data }) => { this.CommentDetails = data });
            },
            AddComment(){
                this.$Progress.start();
                this.form.post('/stud/community/add-comment/article/'+this.articleID);
                this.form.reset();
            ...
            AddReply(commentID){
                this.form2.comment_id = commentID;
            },
            SubmitReply(){
                this.$Progress.start();
                this.form2.post('/stud/community/add-reply/comment/'+this.form2.comment_id);
                this.form2.reset();
                $('#AddReply').modal('hide');
                this.$Progress.finish();
                swal.fire({
            ...
            LoadCommentsAutomatic(){
                this.test = setInterval(()=>this.LoadComments(),3000)
            }
        },
        beforeDestroy () {
        ...
        created(){
            ...
            toast.fire({
                type: 'success',
                title: 'Article '+this.articleID+' Loaded'
            });
            this.LoadCommentsAutomatic();
        }
    }
</script>
